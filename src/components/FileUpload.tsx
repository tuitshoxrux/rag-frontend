...
interface FileUploadProps {
  onUploadSuccess: (totalChunks: number) => void;
}
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response.successful_uploads > 0) {
        onUploadSuccess(response.total_chunks);
      }
    } catch (error: any) {
            ...
            <>
              ...
              <div>
                <p className="text-lg font-semibold text-gray-700">
                  {isDragActive ? 'Drop files here' : 'Word hujjat yuklang'}
                </p>
                <p className="text-sm text-gray-500 mt-2">
                  Fayllarni yuklang (.docx, max 10MB)
                </p>
              </div>
            </>
          )}
